...
def test_get_all_posts(authorized_client, test_posts):
    ...
    def validate(post):
        return schemas.ResponsePostWithVotes(**post)
...
def test_delete_other_user_post(authorized_client2, test_user, test_posts):
...
def test_update_other_user_post(authorized_client2, test_user, test_posts):
    data = {
        "title": "updated title",
        "content": "updated content",
        "id": test_posts[0].id
    }
    res = authorized_client2.put(f"/posts/{test_posts[0].id}", json=data)
    assert res.status_code == 403
